...
| sensor noise sigma=0.10, *denoising* | 0.076 | **0.048** (-37%, outside seed noise, ablation-proven, `jepa_denoise.py`) |
...
stripped out on the clean slice. **So I put the conditions back and measured** (Sec. 1 scorecard, Sec. 6): each
...
**First attempt, and a wrong conclusion.** My minimal GRU-JEPA measured a ~10x gap (0.52 vs 0.05) and I
...
feature-by-time grid, mask the future *state* but **keep the known treatment plan** (UDCA/ERCP tokens, we
...
construction as the baseline, Sec. 5), and auditable; **on the clean slice it *ties*** the constrained
baseline (simpler and more stable), **and on the domain's real stresses it *wins* (Sec. 1, Sec. 6).** **Why only a
tie when clean?** The state is near-deterministic and low-rank (intrinsic effective rank **2.83**), and its
one hidden driver is a static scalar the state already reflects, a GRU recovers it (R^2=0.66), yet an
...
Sec. 3, not *sufficient* to make the latent worth its cost on the clean slice.
...
**149/149** times (delta-S -0.173 vs true -0.174) while creeping up otherwise (`probe_metrics.py`). The
alternatives the brief names I rejected: a *loss penalty* (only discourages) and *projection* (guarantee
lives *outside* the model).

**But zero violations is not on-manifold.** A learned manifold critic (`manifold_critic.py`, AUC 1.0 on
...
C=0, M can't move, `test_invariants.py`). It holds (`corr(delta-M, F*C)` 0.57 -> **0.98**), *halves* M's error
...
- **Accuracy vs the noise floor, not a flatline.** Error is reported beside the irreducible floor (spread
  ...
  reducible error, and there the model beats persist-last **~3x** (0.033 vs 0.096) and predict-mean
  **~4.6x** (`probe_metrics.py`).
- **Generalisation probe (out-of-distribution).** Baseline ratchet MAE at K=24 (in-dist **0.033**):
  held-out susceptibility -> **0.099 (3x)**; unseen late treatment -> **0.031** (unchanged, generalises over
  the *observed* lever); beyond horizon -> **0.100 (3x)**. So: generalises over the observable, fails over
...
- **Domain-stress probes, where JEPA earns its keep (`missing_visits.py`, `jepa_denoise.py`; Sec. 1
  ...
  **Sensor noise** (mechanism: `figures/arch_denoised_anchor.png`)**:** a noise-augmented JEPA forecasts
  ...
  noisy observation**, beating the baseline at every sigma (**-37% at sigma=0.10**, 3 seeds; Sec. 1 table); an
  ...
  *is* the mechanism, one a memoryless model cannot have. It trades "passes exactly through the observed
  ...
  JEPA overtakes at **~12-15 months** (3 seeds; Sec. 1). Each needs training *for* the condition, the right way
...
- **Clinical, decision-grade readouts (`clinical_metrics.py`).** Cirrhosis risk *ranking* is strong
  ...
  **decompensation recall 0.27** (median +14 mo late), **cirrhosis-onset 0/20**, the point estimate
  under-shoots the tail: what aggregate MAE hid, and the argument for Sec. 8's distributional head.
...
- **Structural, auditable for free.** `P(t) = P(t0) + sum of non-negative increments`, a running total,
...
- **Probabilistic forecasting (the biggest gap).** Sec. 6's tail miss is **aleatoric** (susceptibility
  ...
  estimate catches almost none of the tail, Sec. 6) at no accuracy cost; a **persistent-latent CVAE**
...
- **Discrete latents (VQ-JEPA), a research direction, not a guaranteed win.** A VQ codebook would commit
  ...
  built (D28).
- **Validate JEPA where it pays, partly done.** I put the domain's stresses back and JEPA won (Sec. 1/Sec. 6).
...
constraint-valid, on-manifold (critic-measured; the naive GRU drifts, Sec. 5), and auditable, then *measured*
